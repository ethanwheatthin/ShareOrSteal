...
#   1. Read the description in the Canvas assignment to be sure you understand the game.
...
class ExpectedValue (AbstractPlayer):
    def choose_move(self):
        ...
        # otherwise, get proportions of steals and shares from opponent so far
            ...
            opponent_share_probability = sum(1 for x in self.opponent_decision_log if x == CHOICE.SHARE) / len(self.opponent_decision_log)
            opponent_steal_proportion = sum(1 for x in self.opponent_decision_log if x == CHOICE.STEAL) / len(self.opponent_decision_log)
